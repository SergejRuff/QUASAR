...
#'
#' @importFrom torch cuda_is_available torch_tensor nnf_l1_loss optim_adam
#' @examples
#' \dontrun{
#' model <- tape_train(
...
#'
#' @importFrom torch cuda_is_available torch_tensor nnf_l1_loss
#' @keywords internal
#' @noRd
tape_adaptive_stage <- function(model, data_np, step = 300L, max_iter = 5L,
  ...
  optD <- torch::optim_ignite_adam(tape_get_decoder_parameters(model), lr = 1e-4)
  optE <- torch::optim_ignite_adam(tape_get_encoder_parameters(model), lr = 1e-4)
...
tape_adaptive_chunk <- function(model, chunk_x, step = 300L, max_iter = 3L) {
  ...
  optD <- torch::optim_ignite_adam(DW, lr = 1e-4)
  optE <- torch::optim_ignite_adam(c(EW, EB), lr = 1e-4)
